...
const InsuranceSignupForm = ({ closeDialog }) => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      await apiServerClient.fetch('/mailerlite/subscribe', {
        ...
        body: JSON.stringify({ email: formData.email, privacy_consent: true, groupId: '182655303128450982' })
      });
    ...
    try {
      const response = await apiServerClient.fetch('/mailerlite/subscribe', {
        ...
        body: JSON.stringify({
          ...
          role: formData.role, privacy_consent: formData.consent, groupId: '182655303128450982'
        })
      });
      if (!response.ok) {
      ...
      if (data.success) {
        trackGAEvent('stakeholder_signup', { email: formData.email, stakeholder_type: 'insurance' });
          ...
          <motion.form key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex items-start gap-3 pt-1">
              <div className="relative flex items-center mt-0.5">
                <input type="checkbox" name="consent" id="insurance-consent" checked={formData.consent} onChange={handleChange} className="peer h-5 w-5 cursor-pointer appearance-none rounded-md border-2 border-gray-400 bg-white checked:border-amber-700 checked:bg-amber-700 transition-all" />
              ...
              <label htmlFor="insurance-consent" className="text-sm text-gray-600 text-left leading-tight cursor-pointer select-none">
                I agree to the{' '}
...
export default InsuranceSignupForm;