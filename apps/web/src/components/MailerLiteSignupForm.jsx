...
const MailerLiteSignupForm = () => {
  const [step, setStep] = useState(1);
  const [formData, setFormData] = useState({
    email: '', car: '', name: '', city: '', country: '', consent: false
  });
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      await apiServerClient.fetch('/mailerlite/subscribe', {
        ...
        body: JSON.stringify({ email: formData.email, privacy_consent: true, groupId: '179693176552949655' })
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiServerClient.fetch('/mailerlite/subscribe', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: formData.email, name: formData.name, city: formData.city,
          country: formData.country, car: formData.car, privacy_consent: formData.consent,
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <motion.form key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleEmailSubmit} className="space-y-3">
            ...
            {formMessage && <div className="flex items-center gap-2 text-sm p-3 rounded-lg border text-red-500 bg-red-500/10 border-red-500/20"><AlertCircle className="w-4 h-4 flex-shrink-0" /><p>{formMessage.text}</p></div>}
            <motion.button type="submit" disabled={isLoading} whileTap={{ scale: 0.98 }}
              ...
              {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /><span>Processing...</span></> : <><span>Join the Founding List</span><ArrowRight className="w-5 h-5" /></>}
            </motion.button>
          </motion.form>
        )}
          ...
          <motion.form key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex items-start gap-3 pt-1">
              <div className="relative flex items-center mt-0.5">
                <input type="checkbox" name="consent" id="mailerlite-consent" checked={formData.consent} onChange={handleChange} className="peer h-5 w-5 cursor-pointer appearance-none rounded-md border-2 border-gray-400 bg-white checked:border-amber-700 checked:bg-amber-700 transition-all" />
              ...
              <label htmlFor="mailerlite-consent" className="text-sm text-gray-600 text-left leading-tight cursor-pointer select-none">
                I agree to the{' '}
...
export default MailerLiteSignupForm;